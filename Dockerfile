...
ENV LD_LIBRARY_PATH=/opt/sailfish-0.10.1_centos7/lib \
    PATH=/opt/sailfish-0.10.1_centos7/bin:$PATH
...
RUN yum -y update && \
    ...
    cmake -DCMAKE_INSTALL_PREFIX=/opt/sailfish-0.10.1_centos7 -DFETCH_BOOST=TRUE -DNO_RTM=TRUE && \
    make && make install && \
...
ENTRYPOINT [ "/usr/local/bin/run_wrapper.py", "/opt/sailfish-0.10.1_centos7/bin/sailfish" ]
CMD ["--help"]
