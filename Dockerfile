...
ENV LD_LIBRARY_PATH=/opt/sailfish-0.10.0_trusty/lib
...
RUN apt-get update && \
    ...
    cmake -DCMAKE_INSTALL_PREFIX=/opt/sailfish-0.10.0_trusty -DFETCH_BOOST=TRUE -DNO_RTM=TRUE && \
    make && make install && \
...
ENTRYPOINT [ "/usr/local/bin/run_wrapper.py", "/opt/sailfish-0.10.0_trusty/bin/sailfish" ]
CMD ["--help"]
